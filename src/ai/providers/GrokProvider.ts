/**
 * GrokProvider - xAI Grok API Integration
 *
 * xAI Grok API를 통한 텍스트 생성을 구현합니다.
 * Grok은 OpenAI 호환 API 형식을 사용합니다.
 * https://docs.x.ai/api
 */
...
export class GrokProvider extends BaseProvider {
    ...
    async generateText(
        ...
        try {
            const response = await this.makeRequest<GrokResponse>(url, {
                url,
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${apiKey}`,
